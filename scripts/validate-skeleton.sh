...
require_dir apps/ios
require_dir apps/android
require_dir packages
require_dir tests

if find "$root" -path "$root/.git" -prune -o -type f -name '* 2.*' -print | grep -q .; then
  fail "duplicate conflict-style files matching '* 2.*' exist"
else
  ok "no duplicate conflict-style files"
fi

if [[ -f "$root/.env.example" ]] && grep -q '^COP_IOS_MINIMUM_VERSION=26\.0$' "$root/.env.example"; then
  ok "minimum iOS version is pinned to 26.0"
else
  fail "COP_IOS_MINIMUM_VERSION must be pinned to 26.0 in .env.example"
fi
